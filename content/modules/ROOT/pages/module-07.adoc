...
**VS Code Server** will open a _small_ dialog at the top of the window asking for username and password, else git in the terminal will simply ask for username and password.
...
TIP: If you have been logged out or log in for the first time, refer to the xref:lab-access.adoc[Lab Access] page for the access details.
...
Open menu:Automation Execution[Jobs] and select the most recent project sync job for **Signed Project**. If the job succeeded, signature validation succeeded. You can also scroll the job output to find these tasks:
...
**VS Code Server** will again open a _small_ dialog at the top of the window asking for username and password, or git in the terminal will ask for the same information.
In both cases, enter `{gitea_username}` respectively `{gitea_password}`.
