...
You will clone this repository locally to your bastion host and then see how automation controller can be used to check for signed content.

== Prepare Repository

=== Access Gitea

Open the menu:Gitea[] tab in Showroom, or go to {gitea_url}[window=blank] in your browser.

Sign in to **Gitea** as user `{gitea_username}` with the password `{gitea_password}`.

Look around, to the right in the UI you'll see a list of repositories, click the `{gitea_username}/{gitea_controller_project}` link. This will take you to the repository.

=== Clone the repository

Switch to your terminal (how to access a terminal is described xref:index.adoc#_accessing_your_lab_environment[here]) for the next tasks.

Next, you need the repository from Gitea in your home directory on the bastion host. As you have already cloned this repo in the first chapter it should already exist. To check, in your terminal run `ls -l ~`, which should output something like:
...
total 4
drwxr-xr-x. 5 {ssh_username} users 4096 Apr 24 13:10 {gitea_controller_project}
----

If you don't have the git repository, run the following commands:
...
First we have to create the keys we'll use with GPG. Create the file `/home/{ssh_username}/gpg.txt` using your favorite editor with the following content:
...
This creates a key pair _without_ a passphrase (of course, normally you would want to have a passphrase for https://www.redhat.com/sysadmin/creating-gpg-keypairs[your private GPG key,window=blank]).
...
The first step toward signing content is to create a file that tells `ansible-sign` which files to protect. This file should be called `MANIFEST.in` and reside in the project root directory. So go ahead and create the file `/home/{ssh_username}/{gitea_controller_project}/MANIFEST.in` using your favorite editor with the following content:
...
[source,shell,role=execute,subs="attributes"]
----
ansible-sign project gpg-sign /home/{ssh_username}/{gitea_controller_project}
----
...
[source,shell,role=execute,subs=attributes]
----
head -n-0 /home/{ssh_username}/{gitea_controller_project}/.ansible-sign/sha256sum.*
----
...
To make the repository available as a Project to automation controller, the new files must be pushed from our local git repository to the Gitea repository. Add the new files for git staging:
...
**VS Code Server** will open a small dialog at the top of the window asking for username and password, or git in the terminal will ask for the same information.
In both cases, enter `{gitea_username}` and `{gitea_password}`.

image::vscode-git-login.png[800,600,link=self,window=blank]
...
Copy **all** of its contents **including** `-----BEGIN PGP PUBLIC KEY BLOCK-----` and `-----END PGP PUBLIC KEY BLOCK-----`.

In the web UI, open menu:Automation Execution[Infrastructure > Credentials].

. Click btn:[&#x2295; Create credential].
. Set:
** **Name:** `ansible-sign-cred`
** **Organization:** `Default`
** **Credential Type:** `GPG Public Key`
. Paste the full public key from the clipboard into the text field.
. Click btn:[Create credential].
...
To configure the Gitea repository as a project you need an SCM credential. In menu:Automation Execution[Infrastructure > Credentials]:
...
Create the project in menu:Automation Execution[Projects]:
...
** **Content Signature Validation Credential:** `ansible-sign-cred`
** **Source Control URL:** pass:a,q[`{gitea_url}/{gitea_username}/{gitea_controller_project}.git`]
** **Source Control Credential:** `gitea-cred`
. Click btn:[Create project].
...
== Check if signatures were verified in controller

Let's check if the signatures were verified during the initial project sync when you clicked btn:[Create project].

Open menu:Automation Execution[Jobs] and select the most recent project sync job for **Signed Project**. If the job succeeded, signature verification succeeded. You can also scroll the job output to find these tasks:
...
Above tasks indicate that the signature verification was successful.
...
Open the file `/home/{ssh_username}/{gitea_controller_project}/apache_install.yml` in your terminal and change something, like set `enabled: false`.
...
**VS Code Server** will open a small dialog at the top of the window asking for username and password, or git in the terminal will ask for the same information.
In both cases, enter `{gitea_username}` respectively `{gitea_password}`.

If this change was legit, you would have re-signed the repository content. Since we didn't sign it automation controller should recognize the signature of the file is not valid anymore and fail the repository sync. Give it a try:

* In the web UI, open menu:Automation Execution[Projects] and start a sync of **Signed Project** using the circular arrow icon.
* Check the result in menu:Automation Execution[Jobs].
...
Software supply chain security is a very important topic, specially in the context of recent hacks were attackers managed to inject malware into well known and widely used software projects. With the described capabilities of the Red Hat Ansible Automation Platform, you can also create a complete chain of trust for your automation code. When you run mission critical automation, you also have to ensure you only execute trusted, certified and verified automation code.
