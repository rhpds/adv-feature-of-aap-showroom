...
Switch back to the menu:Codeserver[] tab for the next tasks.

Next, you need the repository from Gitea in your home directory on the bastion host. As you have already cloned this repo in the first chapter it should already exist. To check, in your **VS Code Server** terminal run `ls -l ~`, which should output something like:
...
**VS Code Server** will again open a small dialog at the top of the window asking for username and password, or git in the terminal will ask for the same information.
...
Open the file `/home/{ssh_username}/{gitea_controller_project}/apache_install.yml` in your **VS Code Server** and change something, like set `enabled: false`.
