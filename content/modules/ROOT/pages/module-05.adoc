...
Our recommendation in real life remains to use Python to write your dynamic inventory _plugins_ (not scripts!), and benefit from libraries, documentation and examples written in the same language.
And actually you should even place the inventory plugin in a collection and execute it from an Execution Environment loaded into the provided by private automation hub.
...
Go to your terminal (how to access a terminal is described xref:index.adoc#_accessing_your_lab_environment[here]) and use curl to query your "external inventory source":

[source,shell,role=execute,subs="attributes"]
----
curl {gitea_url}/{gitea_username}/{gitea_controller_project}/raw/branch/main/constructed-inventory/inventory_list
----
...
An inventory script (and plugins, too) has to follow some conventions. It must accept the *`--list`* and *`--host <hostname>`* arguments. When it is called with *`--list`*, the script must output JSON-encoded data containing all groups and hosts to be automated. When called with *`--host <hostname>`* it must return an JSON-formatted hash or dictionary of host variables (can be empty) for the specified host.
...
if [ "$1" == "--list" ] ; then
    curl -sS {gitea_url}/{gitea_username}/{gitea_controller_project}/raw/branch/main/constructed-inventory/inventory_list
elif [ "$1" == "--host" ]; then
    echo '{"_meta": {"hostvars": {}}}'
else
    echo "{ }"
...
. Bring up your terminal (for the **VS Code Server** terminal log in with password `{controller_password}`).
. Create the file `/home/{ssh_username}/dyninv.sh` with the content of the Bash script shown above.

TIP: Create the script in `/home/{ssh_username}/`.

. Make the script executable:
+
[source,shell,role=execute,subs="attributes"]
----
chmod +x /home/{ssh_username}/dyninv.sh
----
...
[source,shell,role=execute,subs="attributes"]
----
/home/{ssh_username}/dyninv.sh --list
----

This should give you the same output as above.

As simple as it gets, right? More information can be found on https://docs.ansible.com/ansible/latest/dev_guide/developing_inventory.html[how to develop dynamic inventories,window=blank].
...
. In the AAP web UI, open menu:Automation Execution[Infrastructure > Inventories].
. To create a new inventory, open the btn:[&#x2295; Create inventory] drop-down and click btn:[Create inventory].
. Fill in the fields:
** **Name:** `Cloud inventory`
** **Organization:** `Default`
. Click btn:[Create inventory].
. Open the menu:Sources[] tab and click btn:[&#x2295; Create source].
. Fill in the fields:
...
. Return to the menu:Sources[] tab of the **Cloud inventory**.
. Start the inventory update using the circular arrows icon.

image::inventory-source.png[800,600,link=self,window=blank]

Open menu:Automation Execution[Jobs] to watch the initial sync; **Type** is `Inventory sync`.

After the inventory sync has finished, open menu:Automation Execution[Infrastructure > Hosts] and review the new hosts added to the **Cloud inventory**.
You should see hosts matching the JSON output above.
