...
IMPORTANT: If you've _skipped_ any of the previous chapters, call `cd ~/playbooks-adv-controller && ansible-navigator run add-dyn-inventory.yml` in a terminal to set the pre-requisites for the next steps.

== Create a basis

Let's create our first constructed inventory by opening menu:Automation Execution[Infrastructure > Inventories].
...
. Create a new inventory: click btn:[Create inventory], then choose **Create constructed inventory**.
