...
You might remember the concept of Smart Inventories. They allowed us to create inventories over multiple inventory sources and optionally combine them with some built-in filters. However, smart inventories also had some severe limitations, most notably, smart inventories lost host group membership information.
...
IMPORTANT: If you've _skipped_ any of the previous chapters, run `cd ~/playbooks-adv-controller && ansible-navigator run add-dyn-inventory.yml` in a terminal to set the pre-requisites for the next steps.

== Create a basis

Let's create our first constructed inventory:

. Access your AAP UI from the menu:AAP[] tab in Showroom, or open {controller_url}[window=blank] in a browser tab.
. Log in as `{controller_username}` with password `{controller_password}` (if not already done).
. Open menu:Automation Execution[Infrastructure > Inventories].
. Click btn:[&#x2295; Create inventory], then choose **Create constructed inventory**.
* **Name:** `Constructed Inventory`
* **Organization:** `Default`
* Open the **Input Inventories** drop-down and select:
** [x] Cloud inventory
** [x] Lab inventory
* Close the drop-down list by clicking on an empty area of the UI.
* In **Source variables**, add the following:
+
...
. Click btn:[Sync inventory].
. After the sync has finished, open the menu:Hosts[] tab of the **Constructed inventory**.
You should see all hosts from the `Cloud inventory` and the `Lab inventory` combined.
. In the menu:Groups[] tab you should also see a group called `dyngroup` which was imported from our dynamic inventory `Cloud inventory`.
...
Click btn:[Save inventory], then btn:[Sync inventory], to apply the constructed inventory.
...
. Open menu:Automation Execution[Infrastructure > Inventories] again.
. Edit **Constructed Inventory** using the pencil icon on the right.
. Replace **Source variables** with the following:
+
[source,yaml,role=execute]
----
---
...
. Click btn:[Save inventory] and btn:[Sync inventory] again.
...
Constructed inventories are a great way to automatically organize hosts in groups, apply naming standards and much more. It's worth spending some time experimenting with this new capability.

If you want to learn more about constructed inventories, there is a great https://www.redhat.com/en/blog/how-to-use-the-new-constructed-inventory-feature-in-aap-2.4[Blog Post,window=blank] and of course the https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.6/html/using_automation_execution/controller-inventories#ref-controller-constructed-inventories[Using automation execution documentation,window=blank].
