...
In your browser open {controller_url}[window=blank] (or click on the tab "AAP" on the right of those instructions) and login as user `{z_ansible_controller_admin_username}` with the password `{controller_password}`.
