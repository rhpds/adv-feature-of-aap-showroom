...
In your browser open {controller_url}[window=blank] (or click on the tab menu:AAP[] on the right of this instructions) and login as user `{controller_username}` with the password `{controller_password}`.
...
Click the **Launch workflow** button directly or go to the the **Templates** view and launch the **Deploy Webapplication** workflow by clicking the rocket icon.
