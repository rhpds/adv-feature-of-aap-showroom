...
In your browser open {controller_url}[window=blank] (or click on the tab "AAP" on the right of those instructions) and login as user `{controller_username}` with the password `{controller_password}`.
