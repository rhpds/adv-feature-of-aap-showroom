...
As always it is better to see this in action, so let us get started. Log in to AAP as `{controller_username}` with password `{controller_password}` and open a terminal (check xref:index.adoc#_accessing_your_lab_environment[how to access a terminal]).

The first step concerns the rego policies.
A set of policies has been prepared for you; clone the repository on the bastion host and copy the policies into the OPA server policy directory:
...
====
++++
<div class="solve-button-placeholder" data-module="module-09"></div>
++++
++++
<div class="validate-button-placeholder" data-module="module-09"></div>
++++
====
=====
endif::[]