...
As always it is better to see this in action, so let us get started. Log in to AAP as `{controller_username}` with password `{controller_password}` and open a terminal (how to access a terminal is described xref:index.adoc#_accessing_your_lab_environment[here]).

The first step is the rego policies. A set of policies has been prepared for you; clone the repository on the bastion host and copy the policies into the OPA server policy directory:
