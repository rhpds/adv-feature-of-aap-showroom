...
[WARNING]
====
We are running the OPA server here with no authentication, no TLS/HTTPS and no access controls. This is obviously only suitable for this lab, for using OPA in production your setup has to be more involved.
====
...
* **Decision**: `Allow: true` the job proceeds to execution. `allow: false` the job is blocked, and an error message is returned explaining which policy was violated.
...
** Go to menu:Settings[Automation Execution > Templates] and click the pencil icon on the **Demo Job Template** to edit it
...
* link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.6/html/configuring_automation_execution/controller-pac[AAP Documentation: Implementing policy enforcement]
* link:https://autodotes.com/posts/muid7W8FhABshWX1zwDS[Blog Post: Getting Started with Policy Enforcement for Ansible Automation Platform]
* link:https://play.openpolicyagent.org/[Rego Playground for testing Rego Policies]
