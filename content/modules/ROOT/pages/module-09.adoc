...
And note you have to provide your own OPA server anyway as it isn't part of AAP.
====
...
As always it's better to see this in action, so let's get started! If you haven't already, log in to Codeserver (password **{controller_password}**.) and AAP (user **lab-user**, password **{controller_password}**).
...
** Set the field **Policy enforcement** to `aap/deny`
...
** Change the field **Policy enforcement** to `aap/extravars`
...
** It checks if the current time is inside a given start and end time, this is the maintenance window
** Allows execution of Job Templates in the specified time window only when it has the label `maintenance`.
** Doesn't allow the execution of Job Templates with the label `maintenance` outside the maintenance window

=== Configure the Policy

The policy file has alread been copied to the policy directory. To make sure the current time is inside the time window specified:

** Find the current time in UTC by running `date -u` in your terminal 
** Open the `/var/lib/opa/policies/maintenance_window.rego` in the VS Code editor or your favorite editor
** Find the block with the time specs:
...
Save the file. Now head over to AAP and configure it, we'll use the Demo Job Template again:

** Open the **Demo Job Template** for editing
** Change the field **Policy enforcement** to `aap/maintenance`
** Find the field **Labels**, type `maintenance` into it and hit **Enter**
** **Save job template**
...
** Open the **Demo Job Template** for editing
** In the field **Labels**, hit the x on the label `maintenance` to remove it
** **Save job template**
